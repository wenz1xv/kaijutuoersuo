...
* 点击 **确认 (Enter)** 或直接按 Enter 确认，坐标将自动保存至 `sqinfo.json`。
...
综上，在此只提出一些简单的策略与测试。先说结论，根据数据分析，较少数字的消除优先级高，例如[1,9]的消除优先级大于[1,2,2,5]；横向的消除优先级高于竖向；决定分数上限的是运气，决定分数高低的是策略，例如以下151分的局面，多种策略能达到的分数就有较大区别，最低只有115，最高有151。具体算法见main.py。

当前版本只保留一套主策略：**动作级随机前瞻搜索**。它不再逐个执行旧的固定贪心策略，而是在真实点击前先用前缀和枚举当前所有“矩形内数字和为 10”的合法动作，然后在单个矩形动作粒度上做 rollout 搜索。

主要特点：

* **动作级顺序搜索**：每次 rollout 都是一步一步选择矩形动作，避免“固定排序一跑到底”在后盘被局部最优锁死。
* **后盘强化探索**：当剩余格子变少时，候选池会扩大；同时提高包含 8/9 等高位数字动作的优先级，尽量趁还有 1/2 等低位补数时把高位数字带走。
* **动态预算**：先用快速基线估计局面潜力；低潜力局少搜、快速重开，高潜力局自动延长搜索时间。历史数据里 147 分局面在预算延长后可搜索到 150。
* **缓存回放**：评分阶段搜到的最佳动作序列会缓存，真正执行时直接回放同一条序列，避免预测路线和实际点击路线不一致。
* **历史数据记录**：每局会追加保存到 `历史数组.jsonl`，包括初始 16x10 数组、预测分数、是否达到阈值和动作序列，方便后续继续改进算法。
